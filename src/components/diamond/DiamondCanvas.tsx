...
import type { MeshSettings, ProceduralSettings, StudioSettings } from './types';
...
export function DiamondCanvas({mode,settings}:{mode:'mesh'|'procedural'|'studio';settings:MeshSettings|ProceduralSettings|StudioSettings}){
 ...
 useEffect(()=>{
  ...
  let disposed=false;const create=mode==='mesh'?createMeshDiamond:mode==='studio'?createStudioDiamond:createProceduralDiamond;
 ...
 const loadingName=mode==='procedural'?'Realistic Physical Photon Shader':mode==='studio'?'Monochrome Studio Orbit':'Multi-Bounce Mesh Refraction';
