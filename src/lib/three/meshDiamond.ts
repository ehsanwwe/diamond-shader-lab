...
export function createMeshDiamond(
  ...
  const renderer = rendererFor(canvas),
    ...
    camera = new THREE.PerspectiveCamera(38, 1, 0.05, 30);
  ...
  let target = new THREE.WebGLRenderTarget(1, 1, { type: THREE.HalfFloatType });
  let front: THREE.Mesh | undefined, back: THREE.Mesh | undefined;
  let environment: THREE.Texture | undefined;
  const backMat = new THREE.ShaderMaterial({
    vertexShader: backfaceVertex,
    fragmentShader: backfaceFragment,
    side: THREE.BackSide,
  });
  const uniforms = {
  ...
  new GLTFLoader().load(
    ...
    (g) => {
      ...
      const scale = 1.4 / box.getSize(new THREE.Vector3()).length();
      front = new THREE.Mesh(geometry, frontMat);
      back = new THREE.Mesh(geometry.clone(), backMat);
  ...
  const resize = () => {
    ...
    uniforms.uBackface.value = target.texture;
  ...
  const stop = lifecycle(
    ...
    (t) => {
      ...
      if (front && back) {
        front.rotation.y += s.rotation * 0.003;
